...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="form-group">
            <strong>Mesin:</strong>
            <input type="text" id="mesin" name="mesin" value="{{ $mobil->mesin }}" class="form-control">
        </div>
    </div>
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="form-group">
            <strong>Kapasitas Penumpang:</strong>
            <input type="number" id="kapasitas_penumpang" name="kapasitas_penumpang" value="{{ $mobil->kapasitas_penumpang }}" class="form-control">
        </div>
    </div>
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="form-group">
            <strong>Tipe:</strong>
            <input type="text" id="tipe" name="tipe" value="{{ $mobil->tipe }}" class="form-control">
        </div>
    </div>
    @if ($kendaraan)
    <div class="col-xs-12 col-sm-12 col-md-12">
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <button class="btn btn-primary" id="submit" value="{{ $mobil->_id }}">Submit</button>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#submit").on("click", function() {
            var url = "{{URL('api/mobil/'.$mobil->_id)}}";
            var id =
                $.ajax({
                    url: url,
                    type: "PUT",
